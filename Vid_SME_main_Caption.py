...
selected_gpu = cuda_available_devices[2]  
...
def get_meta_metrics(input_ids, probabilities, log_probabilities, q, r):
    entropies = []
    all_prob = []
    modified_entropies = []
    max_prob = []
...
data_list = {
    "test": ("./video_json/lmms_VDC_test.json", f"./video_json/videos", "video"),
    "train": ("./video_json/caption_mia_whole.json", f"./video_json/videos", "video"),
}
...
for max_frames_num in frame_list:
    model_type = f"vidsme_caption_frm{max_frames_num}"
    save_path = f"./statistics_{model_type}"

    dataset = MLVU(data_dir, data_list)
    label_1_samples = [data for data in dataset if data["label"] == 1]
    label_0_samples = [data for data in dataset if data["label"] == 0]
    sample_1 = random.sample(label_1_samples, min(1027, len(label_1_samples)))
    sample_0 = random.sample(label_0_samples, min(1027, len(label_0_samples)))
    dataset = sample_1 + sample_0
    torch.manual_seed(0)

    model_path = "./checkpoints/Vid-SME-Caption-7B"
    gen_kwargs = {"do_sample": True, "temperature": 0.1, "top_p": None, "num_beams": 1, "use_cache": True, "max_new_tokens": 1024}
    ...
    q_file = f"q_values_caption_frame_{max_frames_num}.npy"
    r_file = f"r_values_caption_frame_{max_frames_num}.npy"
    ...
    for example in tqdm(dataset):
        ...
        #video input
        prompt1="<|im_start|>system\nCarefully watch this video and pay attention to every detail. Based on your observations, answer the given questions.<|im_end|>\n<|im_start|>user\n<image>\n"
        prompt2=inp
        prompt3="<|im_end|>\n<|im_start|>assistant\n"
        ...
        for goal in goal_parts:
            ...
            metrics = get_meta_metrics(input_ids, probabilities, log_probabilities, q_value, r_value)
            metrics_inverse = get_meta_metrics(input_ids_inverse, probabilities_inverse, log_probabilities_inverse, q_value, r_value)
